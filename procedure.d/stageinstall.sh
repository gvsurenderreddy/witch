...
function browserstage3 {
    ...
    if [ "$REPLY" == "y" ]
    then
        $BROWSER http://www.gentoo.org/main/en/mirrors2.xml || $WITCH/color.sh ERROR "hmm things don't seem to have worked. open up your browser yourself, navigate to http://www.gentoo.org/main/en/mirrors2.xml and download to the witch directory at /mnt/$DISTRONAME"
    ...
    then
        $BROWSER -http-proxy $PROX http://www.gentoo.org/main/en/mirrors.xml || $WITCH/color.sh ERROR "hmm things don't seem to have worked. open up your browser yourself, navigate to http://www.gentoo.org/main/en/mirrors2.xml and download to the witch directory at /mnt/$DISTRONAME"
        read -p "ready to continue? (y):" 
        if [ "$REPLY" == "y" ]
        then
            echo "proceeding"
...
function extractstage3 {
    ...
    echo "unpacking your stage3 ($FILE) to /mnt/$DISTRONAME/. this may take some time, please wait."
    
    #ultra basic:
    echo "extracting $1 to /mnt/$DISTRONAME/"
    
    case "$FILE_EXT" in
        tar.bz2) tar -xvjpf /mnt/$DISTRONAME/$FILE -C /mnt/$DISTRONAME/ ;;
        tar.xz) tar -Jxvfp /mnt/$DISTRONAME/$FILE -C /mnt/$DISTRONAME/ ;;
    esac
